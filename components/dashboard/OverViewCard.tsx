            ...
            {overviewData.map(d => (
                <div key={d.id} className='bg-[rgba(15,23,43,0.5)] border border-[rgba(43,128,255,0.2)] py-6.25 px-6 flex justify-between items-start rounded-2xl'>
                    <div className=' flex flex-col gap-4'>
                        ...
                        <span className={`${d.trend.startsWith('-') ? 'text-[#FD244A]' : 'text-[#05DF72]'} text-[14px]`}>{d.trend}</span>
                    </div>
                    <div className={`w-12 h-12 flex items-center justify-center text-[#FFFFFF] rounded-[14px] ${d.bg}`}>
                        {d.icon}
                    </div>
                </div>
            ))}
