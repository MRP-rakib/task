...
function Sidebar({isOpen,setisOpen}:sidebarprops) {
  ...
  return (
   <aside className={`${isOpen?'w-full opacity-100 visible':'w-0 opacity-0 invisible'} bg-[rgba(0,0,0,0.5)] h-screen md:opacity-100 md:visible absolute top-0 left-0 transition-all duration-300 md:w-60.25 md:sticky z-50`}>
     <div className="top-0 left-0 fixed md:sticky px-2 flex flex-col overflow-y-auto w-60.25 h-screen justify-between bg-[#111B3C] border-r border-[#2B7FFF33]">
      ...
      <div className="pt-18 flex flex-col gap-7">
        {menu.map((m) => {
          ...
          return (
            <Link key={m.id} href={m.href} className={`flex items-center py-1 px-2  gap-4 text-[#FFFFFF] 
            ${isActive && 'rounded-xl shadow-[inset_0_1px_18px_2px_#D2EAFF,inset_0_1px_4px_2px_#D2EAFF]'} 
          `}>
