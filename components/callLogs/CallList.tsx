...
function CallList({callinfo,onSelect,activeId}:callListTYpe) {
  ...
  return (
    <div className='w-full flex flex-col gap-3 lg:gap-5.5 '>
      <div className='w-full flex items-center gap-2 border  bg-[rgba(15,23,43,0.5)]  rounded-[14px] h-12.5 px-5 lg:px-17 border-[rgba(43,128,255,0.2)]'>
        <span className=''>
          <Search color='#62748E'/>
        </span>
        <input type="text" placeholder='Search by phone number, issue type...' className='text-[#62748E] text-[12px] md:text-[14px] lg:text-base w-full outline-0' />
      </div>
      <div className=' lg:hidden flex items-center h-12.5 justify-end  gap-3 lg:gap-6'>
      ...
      <div className='bg-[rgba(15,23,43,0.5)] border border-[rgba(43,128,255,0.2)]  rounded-2xl'>
        <div className='p-4 border-b border-[rgba(43,128,255,0.2)] '>
          <h5 className='text-white text-[16px] md:text-[18px] lg:text-[20px]'>Call List</h5>
        </div>
        <div className={` divide-y divide-[rgba(43,128,255,0.2)]`}>
          {callinfo.map(c=>(
            ...
            <div>
              <div className='flex flex-col md:flex-row lg:items-center gap-2 lg:gap-4'>
                ...
                <div>
                  <h6 className='text-white text-[12px] md:text-[14px] lg:text-base'>{c.phoneNumber}</h6>
                  <p className='text-[#90A1B9] text-[10px] md:text-[12px] text-wrap'>{c.dateTime}</p>
                </div>
              </div>
              <div className=' flex flex-col md:flex-row lg:items-center text-[#90A1B9] gap-2 lg:gap-4 pt-4'>
                <span className='flex items-center text-[12px] lg:text-[14px] gap-1'>
                  <Clock3 size={16} />
                  {c.duration}
                </span>
                <span className='flex items-center text-[12px] lg:text-[14px] gap-1'>
                ...
                <span className='px-2 bg-[rgba(81,162,255,0.2)] text-[#51A2FF] text-[12px] lg:text-[14px] text-nowrap'>{c.issueType}</span>
              </div>
            </div>
            <div>
              <span className={`${callTypeStyles[c.callType]||'bg-gray-800'}  border px-2 lg:px-3 py-1 rounded-full text-xs font-medium`}>
              {c.callType}
            </span>
            </div>
